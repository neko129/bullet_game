...
class EnemyPerson extends Person implements Runnable {
	...
	public void moveToMe() {
		if (this.getX() > this.getGp().mp.getX() + 6) {
			this.setLeft(true);
			this.setRight(false);
			this.setDirection("left");
		} else if (this.getX() < this.getGp().mp.getX() - 6){
			this.setLeft(false);
			this.setRight(true);
			this.setDirection("right");
		}

		if (this.getY() - 90 > this.getGp().mp.getY()) {

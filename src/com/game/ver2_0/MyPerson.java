...
class MyPerson extends Person implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	...
	public void mousePressed(MouseEvent arg0) {
		...
		// 不处于瞄准状态，点击鼠标左键，出拳攻击
		if (arg0.getButton() == MouseEvent.BUTTON1 && !this.isAim()) {
	...
	public void keyPressed(KeyEvent e) {
		if ((e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_A)) {
			...
			if (!this.isAim()) {
				// 在瞄准状态，向左走不会面向左，由鼠标位置决定面向哪，所以不处于瞄准状态才改变方法
				this.setDirection("left");
			}
	...
	public void jump() {
		...
		if (this.jumpTime_now < this.jumpTime_sum * Person.jumpSlowConstant) {
			...
			this.jumpTime_now++;
		} else if (this.jumpTime_now == this.jumpTime_sum * Person.jumpSlowConstant) {
			// 跳跃结束
			this.setJumping(false);
			this.setFalling(true);
			this.jumpTime_now = 0; // 恢复当前跳跃时间
	...
	// public boolean isCollision_myPerson(int collisionY) {
	...
	//
	// // 创建我的小人对应的rectangle
	// Rectangle myRtg = new Rectangle(this.getX(), this.getY(), this.getWidth(),
	// this.getHeight());
	//
	// // 遍历与障碍物测试碰撞
	...
	//
	// // 创建障碍物对应的rectangle
	// Rectangle brRtg = new Rectangle(barrier.getX(), barrier.getY(),
	...
	// // 创建障碍物下面对应的rectangle
	// Rectangle brRtg = new Rectangle(barrier.getX(), barrier.getY() +
	// barrier.getHeight(), barrier.getWidth(),
	// 2);
	//
	// // 判断碰撞
	// if (myRtg.intersects(brRtg)) {
